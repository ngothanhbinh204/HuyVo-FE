...
export function initProductMediaScroll() {
  ...
  if (elements.container.classList.contains("is-initialized")) return;
  elements.container.classList.add("is-initialized");
  ...
  const onDragMove = (e) => {
    ...
    const viewportHeight = window.innerHeight;
    const offsetTop = 100;
    const scrollableDist =
      elements.container.offsetHeight - viewportHeight + offsetTop;
    const targetScrollY =
      containerCurrentTop + percent * scrollableDist - offsetTop;
  ...
  elements.cursor.addEventListener("touchstart", onDragStart, {
    passive: false,
  });
...
export function initProductMediaSliderMobile() {
  ...
  const initSwiper = () => {
    ...
    const mainEl = document.querySelector(".product-images");
